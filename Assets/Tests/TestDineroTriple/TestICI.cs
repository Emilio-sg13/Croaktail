...
public class TestICI : MonoBehaviour
{
    ...
    IEnumerator EsperarYActuar()
    {
        ...
        switch (tag)
        {
            // Ejemplo del caso "cliente" en InteractuableConInventario
            case "cliente":
                if (cliente != null)
                {
                        ...
                        coctelSeleccionado.itemName == coctelPedido.itemName)
                    {
                        ...
                        // Se elimina el cóctel del inventario
                        inventario.BorrarItem(selectedIndex, selectedSprite);

                        // Usamos MejorasManager para aplicar dinero triple si está activado.
                        int valorConMejora = TestMejorasManger.Instance.AplicarDineroTriple(coctelSeleccionado.precio);

                        // Se suma el dinero al contador en la UI de cobro
                        GameObject barraUIObj = GameObject.Find("BarraCobroUI");
                        if (barraUIObj != null)
                        {
                            BarraCobroUI barra = barraUIObj.GetComponent<BarraCobroUI>();
                            barra?.AñadirDinero(valorConMejora);
                        }

                        // Se destruye el cliente (servido)
